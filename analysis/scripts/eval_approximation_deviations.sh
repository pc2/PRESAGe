...
while read name operation; do
    toolchain=oneAPI
    sbatch -J eval_deviations_${toolchain}_${name} -o build_out/eval_deviations_${toolchain}_${name}_%j.out -e build_out/eval_deviations_${toolchain}_${name}_%j.out ./scripts/run.sh ./eval/deviations.jl ${toolchain} ${operation} ${name} 0
done
