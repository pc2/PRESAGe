...
for toolchain in Vitis oneAPI; do
    for name in $(jq -r '.groups[] | select(.type != "binary") | .operations[]' ./config.json); do
        sbatch -J eval_max_deviations_${toolchain}_${name} -o eval_max_deviations_${toolchain}_${name}_%j.out -e eval_max_deviations_${toolchain}_${name}_%j.out ./scripts/run.sh ./eval/max_deviations.jl ${toolchain} ${name}
    done
done
