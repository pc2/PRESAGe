...
for toolchain in Vitis oneAPI; do
    for name in $(jq -r '.groups[] | select(.type != "binary") | .operations[]' ./config.json); do
        sbatch -J eval_deviations_${toolchain}_${name} -o build_out/eval_deviations_${toolchain}_${name}_%j.out -e build_out/eval_deviations_${toolchain}_${name}_%j.out ./scripts/run.sh ./eval/deviations.jl ${toolchain} ${name}
    done
done
